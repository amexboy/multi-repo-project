...
(defn create-module [root module branch name opts]
  (let [src (.getPath (io/file root module))
        work-tree-file (io/file root (:modules-root opts) name module)
        work-tree (.getPath work-tree-file)]
    (if (update-repository root module opts)
      (do
        (println module)
        (git-add-worktree src branch work-tree (:base-branch opts))
        (cond
